...
module.exports = function(script, scriptArgs, nodeArgs, opts) {
  ...
  function start() {
    ...
    child.on('exit', function(code) {
      ...
      // SIMPLE FAILSAFE RESTARTS
      // TODO: REPLACE WITH BACKOFF
      const RESTART_LIMIT = 5;
      const RESTART_PERIOD = 10000;
      const stamp = Date.now();
      restartLog.push(stamp);
      const restartsWithinPeriod = restartLog.reduce((sum, restart) => {
      ...
      restartLog.splice(0, restartLog.length - restartsWithinPeriod);
      restart("", true);      
    })
